...
''' steamLib.py - Steam bruteforce, seperate from mainLib to prevent any errors.
...
def twitUserCheck(username):
    try:
        ...
        driver.get("https://www.steamcommunity.com/search/users/#text=" + username)
        assert (("Sorry, that page doesn’t exist!") not in driver.page_source)
        driver.close()
    except AssertionError:
        return 1

def steamBruteforce(username, wordlist, delay):
    driver = webdriver.Firefox()
